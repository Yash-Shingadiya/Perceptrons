...
#Perceptron before filtering out stop words
def PerceptronWithStopWords(iteration,set_of_learning_rates):
    ...
    for learning_rate in set_of_learning_rates:
        ...
        for each in range(iteration):
            ...
            updated_weights_for_ham = update_weights(path_of_all_ham_files, 0.0, bag_of_words, learning_rate, tokenizer, stemmer)
            #Assume spam as positive class, so target = 1
            updated_weights_for_spam = update_weights(path_of_all_spam_files, 1.0, bag_of_words, learning_rate, tokenizer, stemmer)
...
def update_weights(filepath, target, weights, learning_rate, tokenizer, stemmer):
    ...
    for file in filepath:
        ...
        for word in bag_of_words:
            if word not in weights:
                weights[word] = 0.0
            updated_weights = updated_weights + (weights[word]*bag_of_words[word])
